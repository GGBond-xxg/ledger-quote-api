const VERSION = "1.4.0-pages-hardening";
...
async function handleCrypto(url, env, ctx) {
  ...
  if (!ids.length) throw badRequest("ids is required, e.g. /api/crypto?ids=solana,bitcoin&quote=CNY");

  const result = await getCryptoPrices(ids, quote, ctx);
  return jsonResponse({ ok: true, quote, provider: "CoinGecko", items: result.items, cached: result.cached, stale: result.stale || false, updatedAt: result.updatedAt });
}
...
async function getCryptoPrices(ids, targetCurrency, ctx) {
  ...
  const data = await cachedJson(`crypto:${uniqueIds.join(",")}:${vs}`, DEFAULT_CACHE_TTL_SECONDS, () => fetchJson(cgUrl.toString(), { service: "CoinGecko Price" }), ctx);
  ...
  return { items, cached: data.cached, stale: data.stale || false, updatedAt: data.updatedAt };
}
...
async function getFxRate(base, quote, ctx) {
  ...
  if (base === "USDT") {
    if (quote === "USD") {
      ...
      return { rate: usdt.items[0].price, provider: "CoinGecko/tether", cached: usdt.cached, stale: usdt.stale || false, updatedAt: usdt.updatedAt };
    }
    const usdtInQuote = await getCryptoPrices(["tether"], quote, ctx);
    return { rate: usdtInQuote.items[0].price, provider: "CoinGecko/tether", cached: usdtInQuote.cached, stale: usdtInQuote.stale || false, updatedAt: usdtInQuote.updatedAt };
  }
...
function jsonResponse(data, status = 200) {
  return withCors(new Response(JSON.stringify(data, null, 2), { status, headers: { "content-type": "application/json; charset=utf-8", "cache-control": "no-store" } }));
}
